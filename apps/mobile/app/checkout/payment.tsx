    ...
    try {
      ...
      if (intent.stripeConfigured && intent.clientSecret) {
        const init = await initPaymentSheet({
          merchantDisplayName: "Big MO's Bangers",
        ...
        if (presented.error) {
          // User cancelled or payment failed — return to review.
      ...
      // Success (live payment confirmed, or demo mode without Stripe keys).
      setOrderNumber(intent.orderNumber);
      clearCart();
      setStatus("done");
    } catch (e) {
  ...
  if (status === "done") {
    return (
      <View className="flex-1 items-center justify-center bg-slate-50 px-8">
        ...
        {orderNumber && (
          <Text className="mt-1 text-base font-semibold text-brand">
            {orderNumber}
          </Text>
        )}
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50" contentContainerClassName="p-4">
      ...
      <View className="mb-4 rounded-xl bg-white p-4">
        ...
        {params.note ? (
          <Text className="mt-1 text-xs text-slate-500">📝 {params.note}</Text>
        ) : null}
      </View>
